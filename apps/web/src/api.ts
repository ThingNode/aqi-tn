...
async function get<T>(path:string):Promise<T>{const r=await fetch(`${base}${path}`);if(!r.ok)throw new Error(`API ${r.status}`);return r.json()}
export const api={stations:()=>get<Station[]>('/stations'),latest:()=>get<Reading[]>('/readings/latest')};
